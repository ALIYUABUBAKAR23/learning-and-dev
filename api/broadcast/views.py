...
class BroadcastAPI(APIView):
    ...
    def post(self, request):
        ...
        if broadcast_message:
            ...
            return Response(
                data={"message": "could not send Broadcast message. Try again"}, status=status.HTTP_501_NOT_IMPLEMENTED
            )
    ...
    def put(self, request):
        ...
        if not broadcast_id:
            return Response(
                data={"message": "No Broadcast message id specified"}, status=status.HTTP_501_NOT_IMPLEMENTED
            )
